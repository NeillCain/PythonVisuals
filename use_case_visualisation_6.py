...
for i, use_case in enumerate(pivot_table_annotations.index):
    for j, category in enumerate(pivot_table_annotations.columns):
        ...
        # Draw cell with OCPI Object color and bold outline
...
# Bold solid lines for main divisions and dashed internal lines
for x in range(len(pivot_table_annotations.columns) + 1):
    ax1.axvline(x=x, color='black', linestyle='-', linewidth=1.5)
for y in range(len(pivot_table_annotations.index) + 1):
    ax1.axhline(y=y, color='black', linestyle='-', linewidth=1.5)
...
for i, use_case in enumerate(pivot_table_status.index):
    for j, category in enumerate(pivot_table_status.columns):
        ...
        # Draw cell with Data Status color and bold outline
...
# Bold solid lines for main divisions and dashed internal lines
for x in range(len(pivot_table_status.columns) + 1):
    ax2.axvline(x=x, color='black', linestyle='-', linewidth=1.5)
for y in range(len(pivot_table_status.index) + 1):
    ax2.axhline(y=y, color='black', linestyle='-', linewidth=1.5)
...
fig.legend(handles=legend3_handles, title="MVP Scope", bbox_to_anchor=(0.5, -0.05), loc='upper center', ncol=2)

plt.tight_layout()
#plt.show()
plt.savefig('use_case_visualization6.png')