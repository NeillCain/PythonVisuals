# Creating the figure and plotting as before, but adding the legends at the end in sequence
import pandas as pd
...
for i, use_case in enumerate(pivot_table_annotations.index):
    for j, category in enumerate(pivot_table_annotations.columns):
        ...
        #rect_top = patches.Rectangle((j, i + 0.5), 1, 0.5, facecolor=ocpi_color, edgecolor='black', linewidth=1.5)
        #ax.add_patch(rect_top)
        ...
        #rect_bottom = patches.Rectangle((j, i), 1, 0.5, facecolor=status_color, edgecolor='black', linewidth=1.5)
        #ax.add_patch(rect_bottom)
...
plt.title("Use Case Field Category Mapping", fontsize=16)

# Create separate legends for Data Status, OCPI Objects, and MVP scope
legend1_handles = [patches.Patch(color=color, label=label) for label, color in status_colors.items()]
legend2_handles = [patches.Patch(color=color, label=label) for label, color in ocpi_object_colors.items()]
legend3_handles = [
    patches.Patch(color="green", label="In Scope for MVP"),
    patches.Patch(color="yellow", label="Out of Scope for MVP")
]

# Position legends separately on the figure
legend1 = plt.legend(handles=legend1_handles, title="Data Availability", bbox_to_anchor=(1.05, 1), loc='upper left')
ax.add_artist(legend1)  # Ensure this legend is added first
legend2 = plt.legend(handles=legend2_handles, title="OCPI Object", bbox_to_anchor=(1.05, 0.75), loc='upper left')
...
plt.savefig('use_case_visualization4.png')
